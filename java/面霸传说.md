...
**25.如何停止一个线程，使用stop会有什么问题。**

1. 使用退出标志，使线程正常退出，也就是当run方法完成后线程终止。
2. 使用stop方法强行终止，但是不推荐这个方法，因为stop和suspend及resume一样都是过期作废的方法。
3. 使用interrupt方法中断线程。（一般用这种，不是立即停止，而是在当前线程打一个停止标记）
...
redis的定时任务默认是100ms执行一次，如果要修改这个值，可以在redis.conf中修改hz的值。
...
建议优先使用 LRU 算法，也就是，allkeys-lru 或 volatile-lru 策略。
...
结合实际应用的数据总量、热数据的体量，以及成本预算，把缓存空间大小设置在总数据量的 15% 到 30% 这个区间就可以。每课一问
...
[进阶](https://doocs.gitee.io/advanced-java)

[源码](https://github.com/huifer/source-code-hunter)

[优秀的求职者，是如何巧妙应对面试提问呢？](https://blog.51cto.com/xcbeyond/3395769)
