...
**解耦**：核心系统产生一条信息，下游不同的系统需要消费。通过pub/sub机制可以很好的解决耦合性的问题，需要这条消息的去消费，不需要的取消掉，不需要为了明天加个系统加代码明天减个系统减少代码。

**异步**：设备提交实时数据时，通信层提交完数据其实就可以返回，不需要等待结果返回，后端拿到数据后可以扔到消息队列。如果设备提交数据每次都等的话设备基本就处于不可用状态！

**削峰**：因为设备提交数据一般时定时提交，所以流量并不平稳，一般为几分钟条件一次，那么这个几分钟的时间点就是一个流量高峰，后端服务和数据库很可能顶不住高峰时期的流量，所以可以将数据先扔到消息队列里，在里面异步、按照自己的速度去消费消息，防止消息高峰期将服务与数据库打垮
