...
**异步**、**削峰**：设备提交实时数据时，通信层提交完数据其实就可以返回，不需要等待结果返回，后端拿到数据后可以扔到消息队列。因为设备提交数据一般时定时提交，所以流量并不平稳，一般为几分钟条件一次，那么这个几分钟的时间点就是一个流量高峰，后端服务和数据库很可能顶不住高峰时期的流量，所以可以将数据先扔到消息队列里，在里面异步、按照自己的速度去消费消息，防止消息高峰期将服务与数据库打垮
