...
这样我们同时也可以将一部分某个设备实时状态的这种查询压力分担给redis（做缓存，分担数据库的读压力），毕竟redis应付这种key-value查询是非常适合的。如果需要做复杂关联查询，那么我们可以使用状态表与其他表做链接，可以轻松的做到统计、复杂条件查询、分页。
...
[Cache Aside Pattern](https://doocs.gitee.io/advanced-java/#/docs/high-concurrency/redis-consistence?id=cache-aside-pattern)
...
* 缓存雪崩：设置key的过期时间时加上随机值，防止大面积的key同时过期
* 缓存穿透：数据库不存在的值缓存设置为null，或者使用布隆过滤器
* 缓存击穿：缓存过期时更新缓存的方法加上锁（本地锁，分布式锁），同一时间只能有一个请求去更新缓存
...
* 设置锁的过期时间，防止死锁。
* 使用uuid或者其他值防止释放其他人的锁
* 所有操作注意原子性原子性原子性（lua脚本）
* 如果无法把握锁的过期时间，那么可以使用看门狗机制（守护线程不断观察，自动续期，redisson中实现了）

这些都是在单个实例redis中，如果是集群，更复杂，主从同步的问题，主备切换时怎么办
